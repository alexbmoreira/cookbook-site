...
const IconButton = observer(({onClick, size, className, children, ...rest}) => {
  ...
  return (
    <button
      {...rest}
      className={`flex h-7 w-7 rounded-full bg-night-clear justify-center items-center cursor-pointer transition ease-in-out duration-200 active:bg-night ${className || ''}`}
